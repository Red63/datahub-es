...
import com.retail.datahub.es.model.IndexMappingModel;
...
public class EsClient {
    ...
    public void createIndices(String indices){
        if (StringUtils.isBlank(indices)) throw new EsOperationException("indices is empty!");
    ...
    public void createMapping(String _index, String _type, List<IndexMappingModel> mapping) throws IOException {
        ...
        createIndices(_index); //create index
     ...
     * @param indexOrType
     * @return
     * @throws org.nlpcn.es4sql.exception.SqlParseException
     ...
     * @param scrollIds
     */
    public void clearnScroll(String... scrollIds){
        if(scrollIds == null || scrollIds.length == 0) return;
        ClearScrollRequest request = new ClearScrollRequest();
        List<String> ids = new ArrayList<>();
        for (String id: scrollIds){
            ids.add(id);
        }
        request.setScrollIds(ids);
